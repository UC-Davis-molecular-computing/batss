...
def create_ppsim_running_time_data(fn: str, min_pop_exponent: int, max_pop_exponent: int, end_time: float):
    ...
    for pop_exponent in range(min_pop_exponent, max_pop_exponent + 1):
        ...
        n = int(10**pop_exponent)
        ppsim_inits = {m: n}
        sim = bt.Simulation(ppsim_inits, rxns, simulator_method="crn", continuous_time=True, seed=seed)
...
def write_ppsim_count_samples(
    sim: bt.Simulation, pop_exponent: int, trials_exponent: int, species_name: str, final_time: float
) -> None:
...
def plot_rebop_ppsim_histogram(pop_exponent: int, trials_exponent: int, species_name: str, final_time: float):
    ...
    ax.set_title(
        f"count of species ${species_name}$ sampled at time {final_time} "
...
def ppsim_dimerization_crn(pop_exponent: int, seed: int) -> bt.Simulation:
    m, d = bt.species("M D")
    rxns = [
        (m + m >> d).k(1),
        (d >> m + m).k(1),
...
def ppsim_vilar_crn(pop_exponent: int, seed: int) -> bt.Simulation:
    a, da, dap, dr, drp, ma, mr, r, c = bt.species("A DA DAp DR DRp MA MR R C")
    rxns = [
        (da >> dap + a).k(50),
        (dr >> drp + r).k(0.01),
        (dap >> da + a).k(50),
...
def rebop_vilar_with_inits(pop_exponent: int) -> tuple[rb.Gillespie, dict[str, int]]:
    ...
    inits = {"A": 0, "C": 0, "DA": n // 2, "DAp": 0, "DR": n // 2, "DRp": 0, "MA": 0, "MR": 0, "R": 0}
