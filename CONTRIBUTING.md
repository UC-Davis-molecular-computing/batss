...
rendered offscreen with matplotlib (not screen-recorded) by `scripts/make_readme_gifs.py`.

**`make_readme.py --execute` regenerates them for you**, so they cannot go stale when the example changes.
They add about 12 seconds to a ~100 second run.

They are deliberately *not* regenerated by a plain `make_readme.py`, which is a pure conversion: it re-runs
nothing and is byte-for-byte idempotent. Rebuilding the GIFs means re-running the simulations, and the
simulator is not reproducible even with a fixed seed
([#14](https://github.com/UC-Davis-molecular-computing/batss/issues/14)), so they come out different every
time — an unconditional rebuild would rewrite a couple of hundred KB of binary on every conversion. Use
`--execute --no-gifs` to re-run the notebook while leaving them alone, or run
`python scripts/make_readme_gifs.py` on its own to iterate on the GIFs without a full notebook run.
