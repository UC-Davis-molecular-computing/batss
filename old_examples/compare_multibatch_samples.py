...
if False:
    ...
    custom_pyd_path = Path(
        "C:/Dropbox/git/ppsim-rust/python/ppsim/ppsim_rust/ppsim_rust.cp312-win_amd64_bt-n-over-2.pyd"
    )
...
def main():
    ...
    print(
        f"Comparing multibatch to {compare_to} samples with population size "
    ...
    ax.hist(
        [results_ppsim[state], counts_other],
        # bins = np.linspace(int(n*0.32), int(n*.43), 20), # type: ignore
        bins=bins,  # type: ignore
        alpha=1,
        label=["multibatch", compare_to],
    ...
    ax.set_title(
        f"multibatch vs {compare_to} state {state} distribution sampled at time {end_time} ($10^{trials_exponent}$ samples, n=$10^{pop_exponent}$)"
    )
...
def compare_stats():
    ...
    print(
        f'Statistics for count of state "A", comparing {compare_name} and ppsim samples, '
