...
def main3():
    ...
    vol = 50e-6  # 50 uL
    n = bt.concentration_to_count(total_concentration, vol)
    a, b, u = bt.species("A B U")
    approx_majority_rates = [
        (a + b >> 2 * u).k(k1, units=bt.RateConstantUnits.mass_action),
        (a + u >> 2 * a).k(k2, units=bt.RateConstantUnits.mass_action),
        (b + u >> 2 * b).k(k3, units=bt.RateConstantUnits.mass_action),
    ]
    ...
    print(f"{inits=}")
    sim = bt.Simulation(inits, approx_majority_rates, volume=vol, time_units="seconds", seed=0)
    # print('delta:')
    # m = ['a', 'b', 'u']
...
def dsd_oscillator():
    ...
    b1, b2, c1, c2, a1, a2 = bt.species("b1  b2  c1  c2  a1  a2")
    ...
    method = "multibatch"
    sim = Simulation(
        init_config=init_config, rule=all_rps_dsd_rxns, volume=vol, time_units="seconds", simulator_method=method
    )
    print(f"{init_config=}")
    hours = 12
...
def compare_rebop_sequential():
    ...
    # state = 'B'
    # state = 'U'
    bins = np.linspace(int(n * 0.32), int(n * 0.43), 20)
    if pop_exponent == 2:
        bins = np.linspace(0, n, 20)
    ax.hist(
        [results_seq[state], results_rebop[state]],
        bins=bins,  # type: ignore
        alpha=1,
        label=["sequential", "rebop"],
    ...
    ax.set_title(
        f"state {state} distribution at time {end_time} ($10^{trials_exponent}$ samples, n=$10^{pop_exponent}$)"
    )
...
def sample_configs():
    ...
    end_time = 5
    sim = bt.Simulation(inits, approx_majority)
    ...
    # state = 'U'
    ax.hist(
        [results_ppsim[state], results_rebop[state]],
        bins=np.linspace(int(n * 0.32), int(n * 0.43), 20),  # type: ignore
        alpha=1,
        label=["ppsim", "rebop"],
    )  # , density=True, edgecolor = 'k', linewidth = 0.5)
    ax.legend()
...
def main():
    ...
    init = {a: a_init, b: b_init}
    # init = {a: 6, b: 4}
    sim = bt.Simulation(init, approx_majority, seed=1)
    # i, = species('I')
    # epidemic = [ i+u >> 2*i ]
    # init = { i:1, u:9 }
