...
const PRECOMPUTED_SMALL_RATIONAL_LN_GAMMAS: [[f128; MAX_PRECOMPUTED_DENOMINATOR]; MAX_PRECOMPUTED_DENOMINATOR] = [
    [
        0.0,
...
const LG1: f128 = 0.66666666666666666666666666875815545229093281985458066108081113465975841990625437013082687;
const LG2: f128 = 0.39999999999999999999999440533548502210613316290926048049503731640033630216780851704019313;
const LG3: f128 = 0.28571428571428571429094613297509361650334345377104797971745365245556418301332109522197954;
const LG4: f128 = 0.22222222222222221975826738908555894301120591697217296006666261633589966652891750933338693;
const LG5: f128 = 0.18181818181818250245686109265789351993548969174418157011902382456177825291397840701706558;
const LG6: f128 = 0.1538461538460317890154588944351663231610679351082742582556706529319135758482086718167677;
const LG7: f128 = 0.13333333334802997389443791002138961200581615283256823691072831395861433250848751297250711;
const LG8: f128 = 0.11764705759536556953993447310433762572717017723371393550392034866773259858110658340392759;
const LG9: f128 = 0.10526322994030205329396160427129955291121798214743001999261352557788817275781538560620414;
const LG10: f128 = 0.095235140504843371935608507213095115484437687391731985549670821305657519885864575021233968;
const LG11: f128 = 0.087039345837935769603583774487860193007973401003664029391266891327754138987831114010227144;
const LG12: f128 = 0.078493794248594670268165101799026327271815909183231905843461379742199434030144534975373602;
const LG13: f128 = 0.089908734431217458910014676930010764603174233285125307525098603452056911758204218409684487;
...
pub fn ln_f128(x: f128) -> f128 {
    ...
    let t2: f128 = z * (LG1 + w * (LG3 + w * (LG5 + w * (LG7 + w * (LG9 + w * (LG11 + w * LG13))))));
    let r: f128 = t2 + t1;
...
pub fn ln_factorial_manual(k: u64) -> f64 {
    ...
    let k = k as f64;
    let ret = (k + 0.5) * k.ln() - k + (HALFLN2PI + (1.0 / k) * (1.0 / 12.0 - 1.0 / (360.0 * k * k)));
    ret
}
...
pub fn hypergeometric_sample_manual(popsize: u64, good: u64, draws: u64, rng: &mut SmallRng) -> u64 {
    if good > popsize {
...
pub fn hypergeometric_sample_hrua(popsize: u64, good: u64, sample: u64, rng: &mut SmallRng) -> u64 {
    ...
    let var = ((popsize - computed_sample) as f64 * computed_sample as f64 * p * q / (popsize as f64 - 1.0)) as f64; // variance of the distribution
