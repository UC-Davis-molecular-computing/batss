...
Windows file locks, testing CI builds locally), a **"Troubleshooting a broken dev environment"**
section covering the failures seen on a machine whose toolchain has gone stale (dead uv venv, a
stale `.pyd` silently shadowing every rebuild, year-old nightly), and the PyPI release process;
this file only summarizes some of them.
...
- Add `--uv` if the venv was made by uv and has no `pip` (`maturin failed: Failed to find pip`).
- If the copy of the built `.pyd` fails with "used by another process", a **Jupyter kernel with
  ...
  and re-run.
- **If a rebuild appears to have no effect** — Python raises `AttributeError` for a `#[pymethods]`
  method you can see in `src/` — a stale version-tagged `batss_rust.cp3NN-win_amd64.pyd` is
  shadowing the untagged abi3 `batss_rust.pyd` that maturin installs, because CPython resolves
  tagged names first. Diagnose with
  `python -c "from batss.batss_rust import batss_rust as m; print(m.__file__)"`, then rename the
  tagged file aside. Nothing errors when this happens, so suspect it early.
